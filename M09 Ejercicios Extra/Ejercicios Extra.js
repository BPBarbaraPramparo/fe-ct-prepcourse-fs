...
function numberOfCharacters(string) {
  ...
  for (let i = 0; i < string.length; i++) {
    var char = string[i];
    nuevoObjeto[char] = nuevoObjeto[char] + 1 || 1;
  };
  return nuevoObjeto;
...
function capToFront(string) {
   ...
   for (let i = 0; i < string.length; i++) {
     if (string[i] === string[i].toUpperCase()) {
       mayuscula += string[i]; //Es lo mismo que poner mayuscula = mayuscula + string[i] 
     } else {
       minuscula += string[i]; //Es lo mismo que poner minuscula = minuscula + string[i]
     }
   }
